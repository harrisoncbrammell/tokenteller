...
from tokenteller.drivers.datasets import CommonCrawlDatasetDriver, WikipediaCorpusDatasetDriver
...
def test_wikipedia_corpus_driver_reads_jsonl_export():
    # Same JSONL shape as Common Crawl; Wikipedia driver tags source in categories.
    TEST_DATA_DIR.mkdir(exist_ok=True)
    data_path = TEST_DATA_DIR / "wikipedia_corpus_test.jsonl"
    rows = [
        {"id": "1", "text": "hello world", "language": "en", "url": "https://example.com/a"},
        {"id": "2", "text": "namaste duniya", "language": "hi", "url": "https://example.org/b"},
    ]
    ...
    driver = WikipediaCorpusDatasetDriver(str(data_path))
    hindi = list(driver.iter_records(DatasetQuery(filters={"language": "hi"})))

    assert [record.id for record in hindi] == ["2"]
    assert hindi[0].categories["domain"] == "example.org"
    assert hindi[0].categories["source"] == "wikipedia_corpus"
