...
To run the unit tests, install the optional dev dependencies (includes `pytest`):
...
If `python --version` tells you to install from the Store, you either need a real Python install (for example from [python.org](https://www.python.org/downloads/)) or you must turn off the **App execution aliases** for `python.exe` / `python3.exe` under **Settings → Apps → Advanced app settings → App execution aliases**, then use the `python` from your install or virtual environment.
...
- [src/tokenteller/drivers/models/sentencepiece.py](src/tokenteller/drivers/models/sentencepiece.py)
- [src/tokenteller/drivers/datasets/common_crawl.py](src/tokenteller/drivers/datasets/common_crawl.py)
...
- [docs/creating_drivers.md](docs/creating_drivers.md)
