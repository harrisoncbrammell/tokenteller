...
def main() -> None:
    ...
    print(f"Building a pool from the first {POOL_SIZE} OpenSubtitles records...")
    pooled_records = collect_pool_with_progress(source_dataset, pool_query)
    if not pooled_records:
        raise ValueError("No OpenSubtitles records were returned.")
    ...
    dataset = SampledDataset(name="sampled_opensubtitles", records=sampled_records)
    query = DatasetQuery(limit=SAMPLE_SIZE, sample_strategy="head")
    print(f"Selected {len(sampled_records)} random records from the pooled set.")
    ...
    print("Compared Hugging Face tokenizers on random OpenSubtitles records.")
