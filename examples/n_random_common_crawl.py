...
def build_markdown_summary(
    *,
    ...
    lines = [
        ...
        f"- **Pool size**: first {pool_size} streamed documents",
        f"- **Sample size**: {sample_size} random documents",
        f"- **How records were chosen**: we first streamed the first {pool_size} documents from the dataset, then used a fixed random seed (`{SAMPLE_SEED}`) to select {sample_size} documents from that pooled set.",
        "",
        "## Test summary",
        "",
...
def main() -> None:
    ...
    print(f"Building a pool from the first {POOL_SIZE} Common Crawl documents...")
    pooled_records = collect_pool_with_progress(source_dataset, pool_query)
    if not pooled_records:
        raise ValueError("No Common Crawl records were returned.")
    if len(pooled_records) < SAMPLE_SIZE:
        raise ValueError(f"Expected at least {SAMPLE_SIZE} pooled records but found {len(pooled_records)}.")

    sampled_records = random.Random(SAMPLE_SEED).sample(pooled_records, SAMPLE_SIZE)
    dataset = SampledDataset(name="sampled_common_crawl", records=sampled_records)
    query = DatasetQuery(limit=SAMPLE_SIZE, sample_strategy="head")
    print(f"Selected {len(sampled_records)} random documents from the pooled set.")
    ...
    print("Compared Hugging Face tokenizers on Common Crawl records.")
